...
```{r}
NOMBRE_COMPLETO = "Mariana Caballero"
DIGITOS_MATRICULA = 01376544
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):significa que ese item (diapers) al no tener el LHS significa que no hay una relación con RHS que garantice su presencia.
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque no existe una relación directa entre el item beer y el item nuts, no hay ningúna regla en la que se asocien ambos items.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Es la clave única de la sesión a la que pertenecen los datos dentro del dataset, que se encuentran dentro de una misma sesión id.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Ese id varía en los datos y se refiere a los datos que son limpiados de la sesión id.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): si, de alguna manera los datos del data set se relacionan entre si y existen reglas de asociación entre ellos.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): Id: t_bacf06d3-9185-4183-84ea-ff0db51475ce con frecuencia de 902
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 1592
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): 14.6
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**):Cuando una relación entre datos es muy baja, que en este caso eso pasó, no hay reglas de asociación existentes que se puedan relacionar.
...
Que se generan demasiadas reglas de asociación y satura la memoria RAM
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/Q6GDa7GiFnE
